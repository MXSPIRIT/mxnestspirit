...
(function () {
  ...
  $('run').addEventListener('click', function () {
    ...
    function refine(done) {
      ...
      (function step() {
        ...
        hint('Affinage ' + doneRounds + '/' + total + '…');
        try {
    ...
    function finish() {
      if (!best) { best = result; }
      result = best;
      if (!result) { hint('Aucun plan trouvé.', 'err'); lock(false); return; }
      setResult(best);
      draw(best);
      var bad = -1;
      try {
      ...
      hint(nf > 0 ? nf + ' pièce(s) non placée(s) : planche trop courte ou pièce plus large que la laize.'
         : bad > 0 ? bad + ' pièce(s) en contact — écart lame trop faible.'
         : 'Terminé en ' + s + ' s.',
  ...
  function loadPreset(name) {
    var p = PRESETS[name];
    if (!p) {
      hint('Aucune cote connue pour cette machine : règle la forme, la taille et le retrait d\'après un fichier validé, puis clique Mémoriser.');
      return;
    }
